...
public class BotMiddlewareTest extends BaseIntegrationTest {
    ...
    void setUp() throws URISyntaxException {
        when(scrapperClient.registerChat(anyLong())).thenReturn(Mono.error(new WebClientRequestException(new Throwable(),
                HttpMethod.DELETE,
                new URI("127.0.0.1"),
                HttpHeaders.EMPTY)));
    ...
    void assertErrorHandles() {
        ...
        verify(telegramAPI, times(1)).sendMessage(1L, "Link is not supported incorrect or duplicated. Your links: /list");
    }
}
