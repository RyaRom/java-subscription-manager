...
public class RedisUserDataCache implements UserDataCacheRepository {
    ...
    public Mono<Void> updateState(Long chatId, BotState state) {
        return redisReactiveCommandsString.hset(prefix(chatId), "state", state.toString())
            .then();
    }

    @Override
    public Mono<Void> updateLink(Long chatId, String link) {
        return redisReactiveCommandsString.hset(prefix(chatId), "link", link)
            .then();
    }

    @Override
    public Mono<Void> updateFilters(Long chatId, String filters) {
        return redisReactiveCommandsString.hset(prefix(chatId), "filters", filters)
            .then();
    }

    @Override
    public Mono<Void> updateTags(Long chatId, String tags) {
        return redisReactiveCommandsString.hset(prefix(chatId), "tags", tags)
            .then();
    }

    @Override
    public Mono<UserCache> getUser(Long chatId) {
        return redisReactiveCommandsString.hgetall(prefix(chatId))
    ...
    public Mono<Void> clearUser(Long chatId) {
        return redisReactiveCommandsString.del(prefix(chatId))
            .then();
    }
